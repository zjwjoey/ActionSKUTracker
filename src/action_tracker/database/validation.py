...
def _expected(rows: dict[str, list[tuple[int, list[str], list[Any]]]]) -> dict[str, Any]:
    ...
    for _, headers, values in rows["08_LONG_TERM_MASTER"]:
        ...
        localizations[sku] = {
            ...
            "source": "MASTER_08_LONG_TERM_MASTER", "review_status": "LEGACY_UNVERIFIED",
        }
    for _, headers, values in rows["01_SKU_ZH_CURRENT"]:
        sku = _text(_get(headers, values, "SKU"))
        if sku in localizations:
            localizations[sku].update({"name": _text(_get(headers, values, "中文品名")), "cat1": _text(_get(headers, values, "一级类目（中文）")), "cat2": _text(_get(headers, values, "二级类目（中文）")), "spec": _text(_get(headers, values, "规格（中文）")), "description": _text(_get(headers, values, "中文描述")), "details": _text(_get(headers, values, "中文产品详情")), "source": "MASTER_01_SKU_ZH_CURRENT", "review_status": _text(_get(headers, values, "翻译状态")) or "LEGACY_UNVERIFIED"})
    ...
    for _, headers, values in rows["03_PRICE_HISTORY"]:
        sku = _text(_get(headers, values, "SKU"))
        if sku in products:
            prices.append({"sku": sku, "canonical_id": _text(_get(headers, values, "Canonical_ID")), "observed_at": _text(_get(headers, values, "日期"), "日期"), "previous_price": _maybe_float(_get(headers, values, "旧售价 (€)")), "new_price": _maybe_float(_get(headers, values, "新售价 (€)")), "original_price": _maybe_float(_get(headers, values, "原价 (€)")), "change_type": _text(_get(headers, values, "变化类型")), "promotion_raw": _text(_get(headers, values, "促销状态")), "source_file": _text(_get(headers, values, "来源文件")), "source_sheet": _text(_get(headers, values, "来源Sheet")), "raw_json": _row_to_json(headers, values)})
    for _, headers, values in rows["04_EVENT_HISTORY"]:
        sku = _text(_get(headers, values, "SKU"))
        if sku in products:
            events.append({"sku": sku, "canonical_id": _text(_get(headers, values, "Canonical_ID")), "occurred_at": _text(_get(headers, values, "日期"), "日期"), "event_type": _text(_get(headers, values, "事件类型")), "old_value": _text(_get(headers, values, "旧值")), "new_value": _text(_get(headers, values, "新值")), "evidence": _text(_get(headers, values, "备注")), "source_file": _text(_get(headers, values, "来源文件")), "source_sheet": "04_EVENT_HISTORY"})
    ...
    runs = []
    for _, headers, values in rows["05_RUN_LOG"]:
        record = {}
...
def _compare_lists(entity: str, expected: list[dict[str, Any]], actual: list[dict[str, Any]], fields: tuple[str, ...]) -> dict[str, Any]:
    ...
    for index in range(max(len(left), len(right))):
        ...
        for field in fields:
            ...
            if field == "raw_json" and isinstance(target_value, str):
...
def validate_mirror(master_path: Path, db_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        product_fields = ("sku", "canonical_id", "name_es", "cat1_es", "cat2_es", "spec_es", "product_url", "current_price", "historical_min_price", "historical_max_price", "current_status_raw", "first_seen_at", "last_seen_at", "description_es", "details_es", "image_url")
        actual_products = {row["sku"]: dict(row) for row in db.execute("SELECT " + ",".join(product_fields) + " FROM products")}
        localization_fields = ("sku", "language", "name", "cat1", "cat2", "spec", "description", "details", "source", "review_status")
        ...
        actual_prices = [dict(row) for row in db.execute("SELECT sku,canonical_id,observed_at,previous_price,new_price,original_price,change_type,promotion_raw,source_file,source_sheet,raw_json FROM price_history")]
        actual_events = [dict(row) for row in db.execute("SELECT sku,canonical_id,occurred_at,event_type,old_value,new_value,evidence,source_file,source_sheet FROM events")]
        run_fields = ("run_id", "run_date", "started_at", "finished_at", "git_commit", "sitemap_count", "listing_count", "current_count", "new_count", "reappeared_count", "missing_first_count", "missing_continued_count", "offline_count", "price_up_count", "price_down_count", "promo_start_count", "promo_end_count", "qa_status", "commit_status")
        actual_runs = [{field: row[field] for field in run_fields} for row in db.execute("SELECT " + ",".join(run_fields) + " FROM runs")]
        review_fields = ("review_id", "sku", "review_date", "issue_type", "evidence", "candidate_value", "confidence", "suggested_action", "manual_note", "source_row_no")
        ...
        result["field_parity"] = {
            ...
            "price_history": _compare_lists("price_history", expected["price_history"], actual_prices, ("sku", "canonical_id", "observed_at", "previous_price", "new_price", "original_price", "change_type", "promotion_raw", "source_file", "source_sheet", "raw_json")),
            "events": _compare_lists("events", expected["events"], actual_events, ("sku", "canonical_id", "occurred_at", "event_type", "old_value", "new_value", "evidence", "source_file", "source_sheet")),
