...
Branch: `feat/production-operations-v1`, parent main `83e86ba`.
...
Fixture failure-injection coverage proves duplicate triggers block, collection failure never reaches commit, export failure after commit becomes DEGRADED, resume skips successful commit, and OperationsService reflects SQLite. Full regression and CI are required before release. A real production run and Scheduler shadow period remain to be performed by the operator after merge; this document does not claim those live runs.
