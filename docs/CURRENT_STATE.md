...
本机 Ollama `qwen3:8b` endpoint health 可达，但真实 JSON smoke 返回缺少合同要求的 `fields` 包装并出现英文/乱码，按规则记为 `LOCAL_QWEN_NOT_VERIFIED`，不降低 Validator。精确 HEAD `3921d767bb647948b40337cd4773b7d448b8b25b` 的 GitHub Actions run `33464610129` 已通过 Ubuntu/Windows；Localization 测试已实际执行。不合并 main，等待模型合同修复后再复核。
