...
2026-09-01 收口状态：本地 Provider、结构化 Multi-SKU Learning/Promotion、Review Queue 字段适配、否定详情键和事实覆盖检查已实现；目标分支专项测试 32 passed、必测集合 114 passed、全量 396 passed。最终只读审计：5,379 CURRENT、READY 389、REVIEW_REQUIRED 4,990、普通西语残留 1,671、数字事实 mismatch 334、FACT_NOT_COVERED 0、SOURCE_BLOCKED 7、SOURCE_HASH_CHANGED 13、STALE_LOCALIZATION 13、AI eligible 0（生产 AI 关闭）、AI calls 0、AI avoidance 100%。代码层 `LOCALIZATION_V1_CODE_ACCEPTED`；数据层 `LOCALIZATION_DATA_REVIEW_REQUIRED`；Production Apply/Auto Approval 保持关闭。
