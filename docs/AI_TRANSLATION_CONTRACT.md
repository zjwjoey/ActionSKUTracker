...
Final closure (2026-09-01): the local `qwen3:8b` endpoint was reachable, but the real smoke response violated the JSON `fields` contract and was rejected by the validator. This is recorded as `LOCAL_QWEN_NOT_VERIFIED`; production AI remains disabled. Source-damaged fields are never sent to the provider, and promotion always rechecks current PRIMARY source hashes.
