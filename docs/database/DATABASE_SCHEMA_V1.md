...
字段所有权分为两层：`08_LONG_TERM_MASTER` 提供长期身份、历史价格、生命周期和来源基线；
对仍在售的 SKU，`02_SKU_ES_CURRENT` 覆盖当前西语事实字段（名称、一级/二级类目、规格、描述、详情、
图片、当前价格和商品链接）。该覆盖不改变 canonical_id、历史最低/最高价、首次/最后观察日期、
状态或来源追溯；不在 02 中的历史 SKU 继续使用 08 的值。
