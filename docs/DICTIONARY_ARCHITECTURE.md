...
品牌字典保存标准名、别名、确认状态和证据；Resolver 确认品牌后，中文品名按“品牌标准名+牌+商品通用名”生成（例如 `Stanger牌记号笔`）。品牌名只出现一次，官方大小写/数字/型号保持不变。品牌未确认时不得从西语标题猜测，使用通用名并进入审核。若品牌只是系列或型号、且已自然包含在品名中，可不重复加“牌”，但必须有字典或人工证据。此规则只影响中文展示字段，不得改写西语事实字段。
