...
| 空值 | 真正空值；不得用 `N/A`、`None`、0 或猜测值代替。可选 hash 在边界统一为 NULL/`None`（未知），不得把空字符串当作有效摘要 |
...
品牌标准名、别名、确认状态和证据。品牌可以保留原文，不进行机械翻译。中文品名默认
不主动加入品牌名或“牌”字，品牌另行维护；只有品牌、IP、系列或型号是识别商品所必需
的事实时，才按人工/可信证据保留官方写法。技术型号和商品本体中的“牌”不能误删；
品牌未确认不得臆测，相关疑点进入审核。
