...
- implementation feature HEAD：本轮代码提交后记录；文档提交可在其后，仅更新验收文字。
- exact-head CI：需在本轮代码提交推送后记录最新 run；Ubuntu 与 Windows 均须 `SUCCESS`，并确认 allowlist 实际执行 `tests/test_localization_intelligence.py`。
