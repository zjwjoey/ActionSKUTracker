...
certifications, negation, compatibility, and details key/value semantics.
For details preserve source order and duplicate keys. `Voltaje` means voltage,
`Potencia` means power, `Contenido` means content, and boolean/negation values
must retain their meaning. A fluent but semantically wrong key is
`CORRECTED`.
